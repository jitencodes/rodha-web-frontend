...
export function TestSeriesCardV2({ item, className }: TestSeriesCardV2Props) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative flex h-full min-h-[180px] flex-col overflow-hidden rounded-[6px] border border-[#FFEAD6] bg-[#FFF3E8] hover-shine hover:shadow-sm hover:shadow-orange-500/70",
        item.image ? "aspect-square" : "p-5 md:p-6",
        className
      )}
    >
      {item.image ? (
