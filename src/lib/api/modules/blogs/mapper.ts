...
export function mapBlog(blog: BlogApi): BlogViewModel | null {
  ...
  return {
    ...
    metaDescription: blog.metaDescription?.trim() || description,
...
export function mapBlogs(
  ...
  return items
    .map(mapBlog)
    .filter((item): item is BlogViewModel => item !== null);
}
...
): BlogDetailViewModel | null {
  if (!data?.blog) {
    return null;
  }

  const blog = mapBlog(data.blog);
  if (!blog) {
    return null;
  }

  const relatedSource = data.relatedBlogs ?? data.blog.relatedBlogs;
  const relatedPosts = mapBlogs(relatedSource).filter(
